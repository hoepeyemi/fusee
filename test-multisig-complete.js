...
// Complete test of multisig creation with proper CSRF handling
async function testMultisigComplete() {
  console.log('🔐 Complete Multisig Creation Test...\n');
  ...
  try {
    ...
    await testOtherEndpoints(csrfToken);
...
async function testOtherEndpoints(csrfToken) {
  const endpoints = [
    { name: 'API Info', path: '/api/info', method: 'GET' },
    { name: 'Users', path: '/api/users', method: 'GET' },
    { name: 'Vault Status', path: '/api/vault/status', method: 'GET' }
  ];

  for (const endpoint of endpoints) {
    try {
      console.log(`Testing ${endpoint.name}...`);
      const result = await makeRequest(endpoint.path, endpoint.method, csrfToken);
...
// Make HTTP request
function makeRequest(path, method, csrfToken) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'fusee.onrender.com',
      port: 443,
